...
const getSessionCount = (period) => (period === 'full_day' || !period ? 2 : 1);
export const getMyMonthlyOverview = async (req, res) => {
    try {
        ...
        const sumSessions = (rows) => rows.reduce((total, entry) => total + getSessionCount(entry.period), 0);
        const sumSessionsByType = (rows, type) => sumSessions(rows.filter(entry => entry.type === type));
        ...
        res.status(200).json({
            ...
            data: {
                ...
                stats: {
                    registered_sessions: sumSessions(activeEntries.filter(entry => entry.type === 'office' || entry.type === 'remote')),
                    approved_sessions: sumSessions(approvedEntries.filter(entry => entry.type === 'office' || entry.type === 'remote')),
                    ...
                    approved_work_days: new Set(approvedEntries
                        .filter(entry => entry.type === 'office' || entry.type === 'remote')
                        .map(entry => new Date(entry.date).toISOString().slice(0, 10))).size,
