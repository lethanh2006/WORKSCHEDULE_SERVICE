...
export const normalizeWorkRequest = (body) => {
    const type = String(body.type || '');
    if (!REQUEST_TYPES.has(type))
        return { message: 'Loại đơn không hợp lệ.' };
    ...
    if (!PERIODS.has(period))
        return { message: 'Buổi đăng ký không hợp lệ.' };
    const reason = optionalText(body.reason);
    if (!reason)
        return { message: 'Vui lòng nhập lý do.' };
    ...
    const attachmentUrls = Array.isArray(body.attachment_urls)
        ? body.attachment_urls.filter((url) => typeof url === 'string' && Boolean(url.trim()))
        : [];
