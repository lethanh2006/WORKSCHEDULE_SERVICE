...
import type { WorkRequestType } from '../models/WorkRequest.js';
...
export const normalizeWorkRequest = (
  ...
  if (Number.isNaN(startAt.getTime())) return { message: 'Thời gian bắt đầu không hợp lệ.' };

  const endAt = body.end_at ? new Date(String(body.end_at)) : undefined;
  if (endAt && Number.isNaN(endAt.getTime())) return { message: 'Thời gian kết thúc không hợp lệ.' };
  if (endAt && endAt <= startAt) return { message: 'Thời gian kết thúc phải sau thời gian bắt đầu.' };
  ...
  if (reason.length > 1000) return { message: 'Lý do không được quá 1000 ký tự.' };
  ...
  const attachmentUrls = Array.isArray(body.attachment_urls)
    ? body.attachment_urls.filter((url): url is string => typeof url === 'string' && Boolean(url.trim()))
    : [];
  if (attachmentUrls.length > 5) return { message: 'Mỗi đơn chỉ được đính kèm tối đa 5 tệp.' };
  ...
  return {
    data: {
      ...
      ...(managerIdValue ? { manager_id: new mongoose.Types.ObjectId(managerIdValue) } : {}),
